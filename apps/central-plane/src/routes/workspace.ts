...
export function createWorkspaceRoutes(): Hono<{ Bindings: Env }> {
  ...
  app.post("/workspace/export-builder-pack", async (c) => {
    ...
    if (!req.target || !["claude_code", "codex", "both", "web_builder", "handoff"].includes(req.target)) {
      return new Response(JSON.stringify({ ok: false, error: "target_required: claude_code | codex | both | web_builder | handoff" }), { status: 400, headers: { "content-type": "application/json", ...headers } });
    }
