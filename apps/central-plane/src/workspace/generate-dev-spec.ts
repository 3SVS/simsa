...
- 데이터 필드의 default는 **실제 기본값이 있을 때만** 문자열로 쓰고, 없으면 그 키를 아예 생략한다(null·"unknown" 금지).
...
- A data field's default is a string **only when a real default exists**; otherwise omit the key entirely (never null or "unknown").
