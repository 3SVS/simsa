...
const BRIEF_PR_COPY = {
  ko: {
    ...
    titlePrefix: "Simsa 수리 시작점: ",
    heading: "## Simsa 시각 검수 수리 브리프",
    lead: "Simsa가 실제 브라우저로 앱을 열어 확인한 결과 문제가 발견되어, 수리 시작점 브랜치를 만들었습니다.",
    target: "검수 대상",
    verdict: "판정",
    noRecord: "(기록 없음)",
    runIdLabel: "검사 ID",
    caution1: "> **주의: 이 PR에는 자동 적용된 코드 수정이 없습니다.**",
    caution2: "> 아래 수리 지시서(SIMSA-FIX-BRIEF.md와 동일)를 개발 에이전트나 개발자가 이 브랜치에서 실행해 주세요.",
    ...
    briefHeading: "### 수리 지시서",
    briefFileIntro1: "이 파일은 Simsa 시각 검수가 생성한 수리 지시서입니다.",
    briefFileIntro2: "이 브랜치에서 아래 지시서를 그대로 실행한 뒤, 이 파일은 삭제해도 됩니다.",
    briefFileEnvNote: "> 환경 원인 가능성: 코드 수정만으로 완전히 해결되지 않을 수 있어요 — 환경 변수 설정도 확인하세요.",
  },
  en: {
    defaultIntent: "core feature check",
    titlePrefix: "Simsa repair starting point: ",
    heading: "## Simsa visual-inspection repair brief",
    lead: "Simsa opened your app in a real browser, found problems, and created this repair starting-point branch.",
    target: "Inspected",
    verdict: "Verdict",
    noRecord: "(not recorded)",
    runIdLabel: "Check ID",
    caution1: "> **Note: this PR contains no auto-applied code changes.**",
    caution2: "> Have your dev agent or a developer execute the repair brief below (same as SIMSA-FIX-BRIEF.md) on this branch.",
    ...
    briefHeading: "### Repair brief",
    briefFileIntro1: "This file is the repair brief generated by Simsa's visual inspection.",
    briefFileIntro2: "Execute the brief below on this branch as-is; you may delete this file afterwards.",
    briefFileEnvNote: "> Possible environment cause: a code fix alone may not fully resolve this — also check your environment variables.",
  },
};

export function buildRepairPrContent(payload) {
  const loc = payload?.locale === "en" ? "en" : "ko";
  const C = BRIEF_PR_COPY[loc];
  ...
  const bodyLines = [
    ...
    C.lead,
    "",
    `- ${C.target}: ${targetUrl || C.noRecord}`,
    `- ${C.verdict}: ${decision}`,
    ...(visualCheckId ? [`- ${C.runIdLabel}: \`${visualCheckId}\``] : []),
    "",
    C.caution1,
    C.caution2,
    "",
  ...
  if (envCause) {
    bodyLines.push(C.envCause1, C.envCause2, "");
  }
  bodyLines.push(C.briefHeading, "", "```", agentPrompt, "```", "");
